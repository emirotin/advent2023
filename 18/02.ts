...
function run(initialSegments: Segment[]) {
			...
			for (let i = 0; i < segments.length - 1; i++) {
				for (let j = i + 1; j < segments.length; j++) {
					...
					if (contains(s1, s2)) {
						...
						segments.splice(j, 1, ...newSegments);
						j += newSegments.length - 1;
						changed = true;
					}
				}
	...
	const extractDetachedLoops = () => {
		...
		for (let sn = 0; sn < loops.length; sn++) {
			...
			for (const [i, j] of extractions) {
				const newLoop = segments.splice(i, j - i + 1);
				loops.push(newLoop);
			}
		}
	...
	const findIndices = <T>(a: T[], pred: (x: T) => boolean) => [
		...findIndicesGen(a, pred),
	];
